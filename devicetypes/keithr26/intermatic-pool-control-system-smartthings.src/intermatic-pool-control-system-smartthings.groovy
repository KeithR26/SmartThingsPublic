 ...
 *                                  Added P5043 detection (Pool/Spa switch and Heater state vary with P4xxx versus P5043)
 *									Add refresh() to poll() every POLL_REFERSH_FREQ minutes
*/
def getVERSION () {"Ver 4.0.1"}		// Keep track of handler version

metadata {
	definition (name: "Intermatic Pool Control System - SmartThings", author: "KeithR26", namespace:  "KeithR26") {
		capability "Actuator"
		capability "Switch"
		capability "Polling"
...
private process84Event(byte [] payload) {
	...
	log("DEBUG", 1, "+++++ process84Event Firmware-PE653: ${state.firmwareVersion}  state.expansionVersion=${state.expansionVersion}  EXPANSION_5043=${(EXPANSION_5043)}  getP5043=${getEXPANSION_5043()} ")
...
//	log("DEBUG", 1, "+++++ process87Event Firmware-Expansion: ${payload[VER_EXP_MAJOR_87]}.${payload[VER_EXP_MINOR_87]}")
    // Only the P5043 expansion has its own firmware version. 3 = P5043, 0 = either no expansion or a P4xxx expansion
    state.expansionVersion = "${payload[VER_EXP_MAJOR_87]}.${payload[VER_EXP_MINOR_87]}"
...
private List getHeaterEvents(int heat) {
	log("DEBUG", 1, "+++++ getHeaterEvents heat: ${heat}")
	def cmds = []
    def locHeatState
    def extHeatState
...
def List updated() {
    ...
    if (state.expansionVersion == null) { state.expansionVersion = 0 }
...
//	old		                            isComponent: false, componentName: "ep${childNo}", componentLabel: "Switch ${childNo}"])
			                            isComponent: false, componentName: "ep${childNo}", componentLabel: name])
			//Hubitat
...
List getSchedules(endpoint) {
	...
	executeCommands(getSchedulesInternal("$endpoint".toInteger()))
}
...
def List childSetHeatingSetpoint(dni, temp)  {
    ...
    if (temp) {
    	...
    	log("WARN", "childSetHeatingSetpoint called with NULL temperature")
    }
...
def List setSpaMode()          { executeCommands(setSpaModeInternal(), true) }
def List setPoolMode()         { executeCommands(setPoolModeInternal(), true) }
...
def delayBetweenLog(parm, dly=DELAY, responseFlg=false) {
	log("DEBUG", 2, "+++++ delayBetweenLog parm[${parm.size}] dly=$dly responseFlg=${responseFlg}")
	def lst = parm
	def cmds =[]
	def evts =[]
	def devStr = ""
...
//			log("TRACE", "  - ${index}: instanceof physicalgraph.device.HubAction")
			if (cmds) {
...
//			log("TRACE", "  - ${index}: else: HubAction: $l, format()=$fmt")
	...
	if (evts) {
        log("DEBUG", "<<<<< rspFlg=${responseFlg} dly:$dly/${DELAY} ${cmds.size()} Commands, ${evts.size()-cmds.size()} Events${evtStr}${devStr}")
		evts
	} else {
...
private logCommandList(commands) {
	...
	commands.eachWithIndex { cmd, index ->
		...
		if (! (cmd instanceof String || cmd instanceof GString || cmd instanceof physicalgraph.device.HubAction || cmd instanceof java.util.LinkedHashMap)) {
